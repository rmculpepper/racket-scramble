...
(module rawdecimal racket/base
  ...
  (define (string->rawdecimal s)
    (cond [(regexp-match? #rx"^-?(?:[0-9]+(?:[.][0-9]*)?|[.][0-9]+)$" s)
           (define len (string-length s))
           (define (whole-loop index acc)
             (cond [(= index len)
                    (values acc 0)]
                   [(eqv? (string-ref s index) #\.)
                    (frac-loop (add1 index) acc 0)]
                   [else (whole-loop (add1 index) (extend acc (string-ref s index)))]))
           (define (frac-loop index acc prec)
             (cond [(= index len)
                    (values acc prec)]
                   [else
                    (let ([acc* (extend acc (string-ref s index))])
                      (frac-loop (add1 index) acc* (add1 prec)))]))
           (define (digit-value c)
             (- (char->integer c) (char->integer #\0)))
           (define (extend n c)
             (+ (* 10 n) (digit-value c)))
           (if (eqv? (string-ref s 0) #\-)
               (let-values ([(n prec) (whole-loop 1 0)])
                 (values (- n) prec))
               (whole-loop 0 0))]
          [else (values #f #f)]))
