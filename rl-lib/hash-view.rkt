     ...
     (syntax-parse stx
       [(_ name)
        ...
        (for/list ([hvid (in-list hvids)])
          ...
          (define id
            (cond [(null? avail) (wrong-syntax this-syntax "FIXME: none")]
                  [(null? (cdr avail)) (car avail)]
                  [else (wrong-syntax this-syntax "FIXME: multiple")]))
...
(module+ test
  ...
  (= (match p3 [(point x y z) (+ x y z)]) 3)
  (= (match (point 1 2) [(point x y z) (+ x y z)]) 3))
